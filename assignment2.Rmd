...
# plot AUCs
par(mfrow=c(1,1))
roc(bc.dt$diagnosis[train.idx], bc.sel.back.pred, plot=TRUE, legacy.axes=TRUE, main = "Training AUCs", col="brown2", lwd = 1.4)
roc(bc.dt$diagnosis[train.idx], bc.sel.forw.pred, plot=TRUE, legacy.axes=TRUE, add=TRUE, col="deepskyblue3", lwd = 1.4)
...
Comparing the training AUCs  with the test AUCs and commenting on the overfitting of each model:
```{r}
...
roc(bc.dt$diagnosis[train.idx], as.vector(bc.lasso.pred), plot=TRUE, legacy.axes=TRUE, 
    add=TRUE, col="#FF0099FF", lwd = 3)
legend("bottomright", 
       c("backwards elimination: AUC = 0.992", "forward selection: AUC = 0.972", "ridge regression: AUC = 0.978", "lasso regression: AUC = 0.992"), 
       fill = c("#FF9900FF","#33FF00FF","#0066FFFF", "#FF0099FF"))
```
...
backwards elimination: test < train

stepwise selection: test > train
