...
Thi suggsest that `bc.sel.back` is the better model, but `bc.sel.forw` has fewer predictors.

#### Problem 2 (g)

Using model `bc.sel.back` and model `bc.sel.forw` to predict observations in the training set, and from that compute the training AUC:
...
roc(bc.dt$diagnosis[train.idx], bc.sel.forw.pred, plot=TRUE, legacy.axes=TRUE, add=TRUE, col="deepskyblue3", lwd = 1.3)
legend("bottomright", c("backwards elimination: AUC = 0.992", "forward selection: AUC = 0.989"), fill = c("brown2","deepskyblue3"))
```
...
roc(bc.dt$diagnosis[train.idx], bc.sel.back.pred, plot=TRUE, legacy.axes=TRUE, main = "Training AUCs",
    col="#FF9900FF", lwd = 3)
roc(bc.dt$diagnosis[train.idx], bc.sel.forw.pred, plot=TRUE, legacy.axes=TRUE, 
    add=TRUE, col="#33FF00FF", lwd = 3)
...
For the forward selection model, however, the training AUCs is smaller than the testing AUCs, suggesting the model has a higher bias, but lower variance and hence is not overfitted.
...
```
I personally found it very unclear, what we are supposed to test here.
Hence, I did the two things that make the most sense in my eyes.
First I computed the log-likelihoods of the three genetic risk score models.
Then, I performed log-likelihood test of the three models against the null models.
However, until today I can't think of a way how to "test log-likelihood for the predicted probabilities".

#### Problem 3 (h)

Performing a meta-analysis of `GDM.study2.txt` containing the summary statistics from a different study on the same set of SNPs and the results obtained at point (c) 
(hint: remember that the effect alleles should correspond) 
```{r}

```

Producing a summary of the meta-analysis results for the set of SNPs with meta-analysis p-value < 10^−4 sorted by increasing p-value:
```{r}
