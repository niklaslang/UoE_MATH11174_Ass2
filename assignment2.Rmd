...
Implementing a function `holm.bonferroni <- function(results.dt, alpha)` that takes as input an data.table containing the hypotheses tested and corresponding p-values (`results.dt`) and the desired significance level (`alpha`).
It computer the family-wise error rate for all hypotheses and returns `fwer.results.dt`a data table containing the subset of `results.dt` that are significant according to the Holm-Bonferroni method (fwer < `alpha`) ordered by increasing p-value.
```{r}
